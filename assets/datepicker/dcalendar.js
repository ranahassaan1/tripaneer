...
+(function ($) {
  ...
  var months = [
    ...
    DCalendar = function (elem, options) {
      ...
      this.date =
        this.elem.val() === ""
          ? new Date()
          : this.reformatDate(this.elem.val()).date;
      this.viewMode = "days";
      ...
      this.calendar
        .find(".calendar-head-card .calendar-date-wrapper")
        .click(function () {
          that.selected = new Date(
      ...
      this.calendar
        .find(".calendar-date-holder")
        .on("click", ".calendar-dates .date:not(.date.month) a", function () {
          var span = $(this).parent(),
            day = parseInt($(this).text()),
            plus = span.hasClass("pm") ? -1 : span.hasClass("nm") ? 1 : 0,
            selectedDate = new Date(
              that.date.getFullYear(),
              that.date.getMonth() + plus,
              day
            );
  ...
  DCalendar.prototype = {
    ...
    reformatDate: function (date, dateFormat) {
      var that = this,
        format =
          typeof dateFormat === "undefined" ? that.options.format : dateFormat,
      ...
      if (isFullMonth) {
        ...
        firstY =
          firstY < firstM
            ? format.indexOf("y")
            : format.indexOf("y", format.indexOf(month) + month.length);
      } else if (
        (!isDayNoPadding && !isMonthNoPadding) ||
        (isDayNoPadding && !isMonthNoPadding && firstM < firstD)
      ) {
        ...
        if (lastIndexM == format.length - 1) {
          month = date.substring(
            date.indexOf(before, firstM - 1) + 1,
            lastIndex
          );
        } else if (firstM == 0) {
      ...
      if (
        (!isDayNoPadding && !isMonthNoPadding) ||
        (!isDayNoPadding && isMonthNoPadding && firstD < firstM)
      ) {
        ...
        if (lastIndexD == format.length - 1) {
          ...
          day = date.substring(
            date.indexOf(before, firstD - 1) + 1,
            date.indexOf(after, firstD + 1)
          );
        }
      }
      ...
      if (
        (!isMonthNoPadding && !isDayNoPadding) ||
        (isMonthNoPadding &&
          isDayNoPadding &&
          firstY < firstM &&
          firstY < firstD) ||
        (!isMonthNoPadding && isDayNoPadding && firstY < firstD) ||
        (isMonthNoPadding && !isDayNoPadding && firstY < firstM)
      ) {
    ...
    disabledDate: function (date) {
      ...
      if (that.rangeFromEl) {
        ...
        rangeMin =
          fromData.minDate === "today" ? today : new Date(fromData.minDate);
      }
      ...
      if (that.rangeToEl) {
        ...
        rangeMax =
          toData.maxDate === "today" ? today : new Date(toData.maxDate);
      }
      ...
      return (
        ...
        (rangeMin && date < rangeMin) ||
        (rangeMax && date > rangeMax)
      );
    },
    ...
    getMonths: function () {
      ...
      for (var i = 1; i < 4; i++) {
        ...
        for (var a = 0; a < 4; a++) {
          row[a]
            .html(
              "<a href='javascript:void(0);'>" + short_months[count] + "</a>"
            )
            .attr("data-month", count);
          count++;
        }
        rows.push(row);
      }
      ...
      for (var i = 1; i <= 6; i++) {
        ...
        while (day <= days) {
          ...
          if (i === 1 && dayOfWeek === 0) {
            ...
            if (d.getTime() == that.selected.getTime())
        ...
        if (i === 1 || i > 4) {
          ...
          if (i === 1) {
            ...
            for (var a = 6; a >= 0; a--) {
              ...
              week[a]
                .html('<a href="javascript:void(0);">' + pDays-- + "</a>")
                .addClass("pm");

              if (that.disabledDate(pmDate)) week[a].addClass("disabled");

              if (pmDate.getTime() == that.selected.getTime())
                week[a].addClass("selected");
              if (pmDate.getTime() == today.getTime())
                week[a].addClass("current");
            }
          }
          // Last week
          else if (i > 4) {
            ...
            for (var a = 0; a <= 6; a++) {
              ...
              week[a]
                .html('<a href="javascript:void(0);">' + nmStartDay++ + "</a>")
                .addClass("nm");

              if (that.disabledDate(nmDate)) week[a].addClass("disabled");

              if (nmDate.getTime() == that.selected.getTime())
                week[a].addClass("selected");
              if (nmDate.getTime() == today.getTime())
                week[a].addClass("current");
            }
          }
        }
    ...
    getNewMonth: function (dir, isTrigger) {
      ...
      if (that.viewMode === "days") {
        ...
        if (
          isTrigger ||
          that.options.mode === "calendar" ||
          curr.hasClass("months")
        ) {
          that.getDays(that.date, function (dates) {
            if (isTrigger) {
              ...
              cElem
                ...
                .empty()
                .append(dates)
                [dir == "left" ? "prependTo" : "appendTo"](container);
              setTimeout(function () {
        ...
        lblMonth.text(
          months[that.date.getMonth()] + " " + that.date.getFullYear()
        );
    ...
    selectedView: function () {
      ...
      lblMonth.text(
        months[that.selected.getMonth()] + " " + that.selected.getFullYear()
      );
    ...
    create: function () {
      var that = this,
        ...
        cardhead = $(
          '<section class="calendar-head-card"><span class="calendar-year"></span><span class="calendar-date-wrapper" title="Select current date."><span class="calendar-dayofweek"></span>, <span class="calendar-month"></span> <span class="calendar-date"></span></span></section>'
        ),
        container = $('<div class="calendar-container"></div>'),
        calhead = $(
          '<section class="calendar-top-selector"><span class="calendar-prev">&lsaquo;</span><span class="calendar-curr-month"></span><span class="calendar-next">&rsaquo;</span></section>'
        ),
        datesgrid = $(
      ...
      wrapper
        .append(cardhead)
        .append(container)
        .appendTo(mode === "calendar" ? that.elem : overlay);
    ...
    return $(this).each(function () {
      ...
      that
        ...
        .on("keydown", function (e) {
          if (
            ex_keys.indexOf(e.which) < 0 &&
            that.data(DCAL_DATA).options.readOnly
          )
            return false;
        });
